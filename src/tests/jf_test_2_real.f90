...
module jf_test_2_real_mod

    use json_module
    use, intrinsic :: iso_fortran_env , only: error_unit, output_unit, wp => real64, wp32 => real32
...
contains
    ...
    subroutine test_2_real(error_cnt)
    ...
    call json%add(inp, 't032', 0.1_wp32)
    if (json%failed()) then
        call json%print_error_message(error_unit)
        error_cnt = error_cnt + 1
    end if
    call json%add(inp, 'tf32', 1.1_wp32)
    if (json%failed()) then
        call json%print_error_message(error_unit)
        error_cnt = error_cnt + 1
    end if
    call json%add(inp, 'x032', 9999.000_wp32)
    if (json%failed()) then
        call json%print_error_message(error_unit)
        error_cnt = error_cnt + 1
    end if
    ...
    call add_variables_to_input(json, traj, 'Rx', 'km', 'J2000', 'EARTH', [1.0_wp, 2.0_wp, 3.0_wp], [1.0_wp32, 2.0_wp32, 3.0_wp32], error_cnt )
    call add_variables_to_input(json, traj, 'Ry', 'km', 'J2000', 'EARTH', [10.0_wp, 20.0_wp, 30.0_wp], [10.0_wp32, 20.0_wp32, 30.0_wp32], error_cnt )
    call add_variables_to_input(json, traj, 'Rz', 'km', 'J2000', 'EARTH', [100.0_wp, 200.0d0, 300.0_wp], [100.0_wp32, 200.0_wp32, 300.0_wp32], error_cnt )
    call add_variables_to_input(json, traj, 'Vx', 'km/s', 'J2000', 'EARTH', [1.0e-3_wp, 2.0e-3_wp, 3.0e-3_wp],  [1.0e-3_wp32, 2.0e-3_wp32, 3.0e-3_wp32], error_cnt )
    call add_variables_to_input(json, traj, 'Vy', 'km/s', 'J2000', 'EARTH', [2.0e-3_wp, 20.0e-3_wp, 3.0e-3_wp],  [2.0e-3_wp32, 20.0e-3_wp32, 3.0e-3_wp32], error_cnt )
    call add_variables_to_input(json, traj, 'Vz', 'km/s', 'J2000', 'EARTH', [3.0e-3_wp, 30.0e-3_wp, 40.0e-3_wp], [3.0e-3_wp32, 30.0e-3_wp32, 40.0e-3_wp32],error_cnt )
    nullify(traj)
    ...
    end subroutine test_2_real

    subroutine add_variables_to_input(json, me, variable, units, frame, center, rdata, rdata32, error_cnt)
    !Used by test_2_real.
    ...
    real(wp32),dimension(:),intent(in) :: rdata32
    ...
    !trajectory [vector of reals]:
    call json%add(var, 'DATA32', rdata32)
    if (json%failed()) then
        call json%print_error_message(error_unit)
        error_cnt = error_cnt + 1
    end if
...
end module jf_test_2_real_mod
!*****************************************************************************************

!*****************************************************************************************
program jf_test_2_real

    !! Second unit test.

    use jf_test_2_real_mod , only: test_2_real
    implicit none
    integer :: n_errors
    n_errors = 0
    call test_2_real(n_errors)
    if (n_errors /= 0) stop 1

end program jf_test_2_real
!*****************************************************************************************
