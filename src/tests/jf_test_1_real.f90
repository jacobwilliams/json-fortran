...
contains
    ...
    subroutine test_1_real(error_cnt)
    ...
    if (json%failed()) then    !if there was an error reading the file
      ...
      if (json%failed()) then
        ...
        write(error_unit,'(A,E30.16)') '[8-byte real] data(2).real = ',rval
      end if

      write(error_unit,'(A)') ''
      call json%get('data(2).real', r32val)
      if (json%failed()) then
        call json%print_error_message(error_unit)
        error_cnt = error_cnt + 1
      else
        write(error_unit,'(A,E22.8)') '[4-byte real] data(2).real = ',r32val
      end if
    ...
    end subroutine test_1_real

end module jf_test_1_real_mod
!*****************************************************************************************

!*****************************************************************************************
program jf_test_1_real

    !! First unit test.

    use jf_test_1_real_mod , only: test_1_real
    implicit none
    integer :: n_errors
    n_errors = 0
    call test_1_real(n_errors)
    if (n_errors /= 0) stop 1

end program jf_test_1_real
!*****************************************************************************************
