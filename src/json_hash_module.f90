!*****************************************************************************************
!>
!  A simple hash table implementation for a JSON structure.
!
...
!  pointers to the associated JSON values.
    ...
    subroutine hash_table_create_from_value(me, json, p, status_ok, initial_capacity)
    ...
    status_ok = .true. ! will be set to false if any problems are encountered
    ...
    if (n_members > 0_IK .and. var_type == json_object) then
        ...
        if (.not. status_ok) then
            call set_error() ! should never happen since we know the size
            return
        end if
        do i = 1, n_members
    ...
    end subroutine hash_table_create_from_value
!*****************************************************************************************
